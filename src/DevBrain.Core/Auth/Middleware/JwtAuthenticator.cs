...
namespace DevBrain.Core.Auth.Middleware;
...
///
/// <para>
/// Held separate from host-specific authentication middleware so the acceptance gates can be
/// covered by unit tests without constructing a Functions or ASP.NET Core request context.
/// </para>
///
...
public sealed class JwtAuthenticator
{
    ...
    public async Task<AuthenticationResult> AuthenticateAsync(string? authorizationHeader)
    {
        ...
        foreach (var role in upstreamRecord.Roles)
        {
