...
public sealed class EntraOAuthClient : IUpstreamOAuthClient
{
    ...
    private async Task<UpstreamTokenResponse> PostTokenAsync(Dictionary<string, string> form)
    {
        ...
        var (upn, oid, tid, roles) = await ValidateAndExtractIdTokenClaimsAsync(payload.IdToken);
    ...
    /// </para>
    /// </summary>
    private async Task<(string Upn, string Oid, string Tid, IReadOnlyList<string> Roles)> ValidateAndExtractIdTokenClaimsAsync(string idToken)
    {
