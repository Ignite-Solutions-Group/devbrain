...
public sealed class AuthorizationHandler
{
    ...
    public async Task<AuthorizationResult> HandleAsync(AuthorizationRequest request)
    {
        ...
        if (!string.IsNullOrEmpty(request.Resource)
            && !string.IsNullOrEmpty(request.CanonicalResource)
            && !string.Equals(request.Resource, request.CanonicalResource, StringComparison.Ordinal))
        {
            _logger?.LogWarning("AuthorizationHandler: rejected — resource does not match canonical MCP resource");
            return AuthorizationResult.Error("invalid_target", "resource must identify this DevBrain MCP endpoint.");
        }
        ...
        var transaction = new AuthTransaction
        {
            ...
            Resource = request.CanonicalResource ?? request.Resource ?? string.Empty,
...
/// <summary>Query-string inputs to <c>/authorize</c>, flattened into a host-neutral record.</summary>
